...
def findLengthOfTransformation(start, end, wordDict):
    ...
    'q','r','s','t','u','v','w','x','y','z']
    startWord = WordNode(start, 0, None)
    ...
    while len(bfsQ) > 0:
        ...
        for i in range(0, len(top.word)):
            for c in alpha:
                ...
                if str1 in wordDict:
                    newWord = WordNode(str1, top.numSteps+1, tmp)
                    bfsQ.append(newWord)
                    del wordDict[str1]
                    print str1, newWord.numSteps, newWord.pre
                top.word = tmp

print findLengthOfTransformation('hit', 'cog', {'hot':1, 'dot':1, 'dog':1, 'cog':1, 'lot':1, 'log':1, 'bob':1, 'rot':1})
